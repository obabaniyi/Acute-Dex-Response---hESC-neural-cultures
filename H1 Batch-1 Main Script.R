...
outputPrefix <- "DEX_H1-neuron_DESeq2_batch1"

sampleFiles <- c("C1-25-output_basename.counts",
                 "C2-26-output_basename.counts",
                 "C3-27-output_basename.counts",
                 "D1-28-output_basename.counts",
                 "D2-29-output_basename.counts",
                 "D3-30-output_basename.counts")

sampleNames <- c("C1-25","C2-26","C3-27",
                 "D1-28","D2-29","D3-30")

sampleCondition <- c("6hr_DMSO","6hr_DMSO","6hr_DMSO",
...
pca1 <- (ggplot(pcaData, aes(x = PC1, y = PC2, label = rownames(pcaData), col = condition))
         ...
         + ggtitle("PCA plot all H1 Libraries"))

print(pca1)

ggsave("H1-acuteDex-PCA-plot-allLibs.svg", plot=pca1, width =6.5, height=5, unit='in')
...
##The note above reference a duplicate originally generated on the H9 Batch1 analysis
##Duplicate related to the same gene was generated in the H1 batch1 data set today 4/9/21
##This time the duplicate was found in a different part of the array, so manual removal was necessary 
##I initially thought 
...
de <- as.numeric(resdata_1_go$padj < 0.05)
names(de) <- resdata_1_go$GeneID
de[is.na(de)] = 0
...
GO.wall$gene_ratio <- GO.wall$numDEInCat/GO.wall$numInCat
...
ggsave("Top20Enriched_Go-terms_H1_batch1.svg", plot=gg2, width = 6.5, height = 7, unit = 'in')

#output ggData2-top20 GOseq result file as a table
write.table(ggData2, 
...
#read the file witn GeneID of DEgenes shared in common with H9 Batch
...
resdata_commonGenes <- resdata_commonGenes[, colnames(resdata_commonGenes)[c(1:10,17,11:16)]]

#write out result table
write.table(resdata_commonGenes, 
            file = paste0(outputPrefix, "DEgenes_shared_with_H9_Batch.txt"),
            row.names = FALSE,
            sep = '\t')
...
#rearrange result columns to specification
resdata_commonGenes_all <- resdata_commonGenes_all[, colnames(resdata_commonGenes_all)[c(1:10,17,11:16)]]
...
ggsave("H1_batch1_commonGenes.svg", plot = ggCommon_all, width = 3, height = 11, unit = 'in')


######################################################################################################
#plot fold change and direction of genes common to H9 batch1 and H1 batch 1 only 

#read the table of genes common to H1 and H9 from a saved file
commonGenes_H1_H9 <- fread("DEX_H1-neuron_DESeq2_batch1-DEgenes_shared_with_H9_Batch.txt")

#reorder the table using shrunkenlogfold2 as factor
commonGenes_H1_H9$hgnc_symbol <- factor(commonGenes_H1_H9$hgnc_symbol, levels = commonGenes_H1_H9$hgnc_symbol[order(commonGenes_H1_H9$ShrunkenLog2FoldChange)])

mycols_common2 <- rep("gray", nrow(commonGenes_H1_H9))
mycols_common2[ commonGenes_H1_H9$Direction == "up" ] <- "red"
mycols_common2[ commonGenes_H1_H9$Direction == "down" ]  <- "blue" 

ggCommon_H1_H9 <- ggplot(commonGenes_H1_H9, aes(x=hgnc_symbol, y=ShrunkenLog2FoldChange))+
...
ggsave("H1_batch1_commonGenes2.svg", plot = ggCommon_H1_H9, width = 1.5, height = 5, unit = 'in')
...
pathways_all <- gmtPathways("msigdb.v7.4.symbols.gmt")

#turns gene rank file into a named vector
ranks <- deframe(gsas_prerank_file4)

#does the fgsea analysis 
fgseaRes <- fgsea(pathways=pathways_all, stats=ranks, eps=0)
...
plotGseaTable(pathways_all[topMainPathways], ranks, fgseaRes, 
              gseaParam=0.5)
...
responseCort <- plotEnrichment(pathways_all[["GOBP_CELLULAR_RESPONSE_TO_CORTICOSTEROID_STIMULUS"]],
                               ranks) + labs(title="H1_BATCH1 GO:BP CELLULAR RESPONSE TO CORTICOSTEROID STIMULUS")

ggsave("H1 batch1 GOBP-response to corticosteroid stimulus plot.svg", plot = responseCort, width = 3, height = 2, unit = 'in')


#script to plot the fan embryonic CTX ex4 excitatory neuron
plotEnrichment(pathways_all[["FAN_EMBRYONIC_CTX_EX_4_EXCITATORY_NEURON"]],
               ranks) + labs(title="FAN EMBRYONIC CTX EX4 EXCITATORY NEURON")

######################################################################################################

#script to plot the GOCC set for synapes
plotEnrichment(pathways_all[["GOCC_SYNAPSE"]],
               ranks) + labs(title="GOCC SYNAPSE")

#script to plot the GOCC set for anchoring junction
plotEnrichment(pathways_all[["GOCC_ANCHORING_JUNCTION"]],
               ranks) + labs(title="GOCC ANCHORING JUNCTION")

#script to plot the GOCC set for endoplasmic reticulum
plotEnrichment(pathways_all[["GOCC_ENDOPLASMIC_RETICULUM"]],
               ranks) + labs(title="GOCC ENDOPLASMIC RETICULUM")

######################################################################################################

#script to plot a sample of positively and negatively enriched genesets for this dataset. 

#script to plot the GOCC set for CHEN_LVAD_SUPPORT_OF_FAILING_HEART_UP
plotEnrichment(pathways_all[["CHEN_LVAD_SUPPORT_OF_FAILING_HEART_UP"]],
               ranks) + labs(title="CHEN LVAD SUPPORT OF FAILING HEART UP")


#script to plot the GOCC set for HALLMARK_ALLOGRAFT_REJECTION
plotEnrichment(pathways_all[["HALLMARK_ALLOGRAFT_REJECTION"]],
               ranks) + labs(title="HALLMARK ALLOGRAFT REJECTION")

######################################################################################################

#script to plot the most enriched GO_cC terms in this dataset. 

#script to plot the GOCC set for GOCC_CHROMATIN
plotEnrichment(pathways_all[["GOCC_CHROMATIN"]],
               ranks) + labs(title="GOCC CHROMATIN")

#script to plot the GOCC set for GOCC_NUCLEAR_BODY
plotEnrichment(pathways_all[["GOCC_NUCLEAR_BODY"]],
               ranks) + labs(title="GOCC NUCLEAR BODY")

#script to plot the GOCC set for GOCC_ORGANELLE_SUBCOMPARTMENT
plotEnrichment(pathways_all[["GOCC_ORGANELLE_SUBCOMPARTMENT"]],
               ranks) + labs(title="GOCC ORGANELLE SUBCOMPARTMENT")